function rho = calcAtmDensity(rEci, params)
    % Arguements:
    % rEci - (double[3x1], km) positional vector of spacecraft in ECI
    % Output:
    % rho  - atmospherical density in model CIRA72, kg / m3
    h = vecnorm(rEci) - params.earthRadius;
    ...
    intervalNumber = sum(h' <= baseAltitudes, 2) + 1;
    ...
    rho = nominalDensity * exp((baseAltitude - h) / scaleAltitude);
 end